...
no_support = 'no_support'
  ...
  var zoom_holder = {
    ...
    cmd: function(val) {$.ajax({url: root+'/ptz?zoom='+val, method: "POST"}).fail(display_error('cannot_set', 'zoom'));}
  };
  ...
  var focal_length_holder = {
    ...
    cmd: function(val) {$.ajax({url: root+'/settings/focal_length?set='+config.avail.focal_length[val], method: "POST"}).fail(display_error('cannot_set', 'focal_length'));}
  };
  ...
  var quality_holder = {
    ...
    cmd: function(val) {
      ...
      $.ajax({url: root+'/settings/quality?set='+val, method: "POST"}).fail(display_error('cannot_set', 'quality'));
    }
  };
  ...
  for (i in crops) {
    ...
    crop_holders.push({
      ...
      cmd: function(control){ return function(val) {
        ...
        $.ajax({url: root+'/settings/' + control + '?set='+val, method: "POST"}).fail(display_error('cannot_set', control));
      }} (control)
    })
  }
  ...
  var molimit_holder = {
    ...
    cmd: function(val) {
      ...
      $.ajax({url: root+'/settings/motion_limit?set='+val, method: "POST"}).fail(display_error('cannot_set', 'motion_limit'));
    }
  };
  ...
  var exposure_holder = {
    ...
    cmd: function(val) {$.ajax({url: root+'/settings/exposure?set='+config.avail.exposure[val], method: "POST"})
  ...
  var nvavg_holder = {
    ...
    cmd: function(val) {
      $.ajax({url: root+'/settings/night_vision_average?set='+val, method: "POST"}).fail(display_error('cannot_set', 'night_vision_average'));
    }
  };
  ...
  var nvgain_holder = {
    ...
    cmd: function(val) {
      $.ajax({url: root+'/settings/night_vision_gain?set='+val, method: "POST"}).fail(display_error('cannot_set', 'night_vision_gain'));
    }
  };
  ...
  var focus_distance_holder = {
    ...
    cmd: function(val) {
      $.ajax({url: root+'/settings/focus_distance?set='+val, method: "POST"}).fail(display_error('cannot_set', 'focus_distance'));
    }
  };
  ...
  var iso_holder = {
    ...
    cmd: function(val) {
      $.ajax({url: root+'/settings/iso?set='+val, method: "POST"}).fail(display_error('cannot_set', 'iso'));
    }
  };
  ...
  var exposure_ns_holder = {
    ...
    cmd: function(val) {
      $.ajax({url: root+'/settings/exposure_ns?set='+val, method: "POST"}).fail(display_error('cannot_set', 'exposure_ns'));
    }
  };
  ...
  var frame_duration_holder = {
    ...
    cmd: function(val) {
      $.ajax({url: root+'/settings/frame_duration?set='+val, method: "POST"}).fail(display_error('cannot_set', 'frame_duration'));
    }
  };
  ...
  var bindUpdEvents = function(control, holder) {
    let upd = scheduleUpdateW(holder)
    control.on('change', upd)
    let ctrlState = {hasMouse: false}
    control.on('mousemove', () => {
      if (ctrlState.hasMouse) {
        upd()
      }
    })
    control.on('mousedown', () => {ctrlState.hasMouse = true})
    control.on('mouseup', () => {ctrlState.hasMouse = false})
    return control
  }
  ...
  var circ_setter = function(x) {
    return function() {
      ...
      $.ajax({url: root+'/settings/video_chunk_len?set='+x, method: "POST"});
    }
  }
  ...
  var reconfigure = function () {
    ...
    config.avail = config.avail || {}
    if (config.avail.rtsp_audio_formats.indexOf('aac') == -1) {
      $($('[name=audio_mode_sel][value=html5_aac]')[0].parentElement).hide()
    }
    ...
    if (config.avail.zoom == undefined) {
      ...
      bindUpdEvents($('#range_zoom')
        .attr('min',0)
        .attr('max',config.avail.zoom.length-1)
        .attr('value', config.avail.zoom.indexOf(config.curvals.zoom)), zoom_holder)
    ...
    if (config.avail.focal_length == undefined || config.avail.focal_length.length < 2) {
      ...
      bindUpdEvents($('#range_focal_length')
        .attr('min',0)
        .attr('max',config.avail.focal_length.length-1)
        .attr('value', config.avail.focal_length.indexOf(config.curvals.focal_length)), focal_length_holder)
      ...
      var flashcb = $('#flashcb')
        .change(function() {
          if (flashcb.is(":checked")) {
            $.ajax({url: root+'/enabletorch', method: "POST"}).fail(display_error('cannot_set', 'torch'))
          } else {
            $.ajax({url: root+'/disabletorch', method: "POST"}).fail(display_error('cannot_set', 'torch'))
          }
      })
    ...
    var focuscb = $('#focuscb')
      .change(function() {
        if (focuscb.is(":checked")) {
          $.ajax({url: root+'/focus', method: "POST"}).fail(display_error('cannot_set', 'focus'))
        } else {
          $.ajax({url: root+'/nofocus', method: "POST"}).fail(display_error('cannot_set', 'focus'))
        }
      })
    ...
    var overlaycb = $('#overlaycb')
      .change(function() {
        if (overlaycb.is(":checked")) {
          $.ajax({url: root+'/settings/overlay?set=on', method: "POST"}).fail(display_error('cannot_set', 'overlay'))
        } else {
          $.ajax({url: root+'/settings/overlay?set=off', method: "POST"}).fail(display_error('cannot_set', 'overlay'))
        }
      })
    ...
    var nvcb = $('#nvcb')
      .change(function() {
        if (nvcb.is(":checked")) {
          $.ajax({url: root+'/settings/night_vision?set=on', method: "POST"}).fail(display_error('cannot_set', 'night_vision'))
        } else {
          $.ajax({url: root+'/settings/night_vision?set=off', method: "POST"}).fail(display_error('cannot_set', 'night_vision'))
        }
      })
    ...
    if (config.curvals.crop_x != undefined) {
      bindUpdEvents($('#range_crop_x')
        .val(config.curvals.crop_x), crop_holders[0])
      bindUpdEvents($('#range_crop_y')
        .val(config.curvals.crop_y), crop_holders[1])
    } else {
      $('#crop_host').hide()
    }
    bindUpdEvents($('#range_quality')
      .val(config.curvals.quality), quality_holder)

    if (config.curvals.exposure != undefined) {
      bindUpdEvents($('#range_exposure')
        .attr('min',0)
        .attr('max',config.avail.exposure.length-1)
        .val(config.avail.exposure.indexOf(config.curvals.exposure)), exposure_holder)
    ...
    if (config.curvals.focus_distance != undefined && config.avail.focus_distance.length >= 2) {
      ...
      bindUpdEvents($('#range_focus_distance')
        .attr('min',minVal)
        .attr('max',maxVal)
        .val(curVal), focus_distance_holder)
    ...
    if (config.curvals.iso != undefined && config.avail.iso.length >= 2) {
      ...
      bindUpdEvents($('#range_iso')
        .attr('min',minVal)
        .attr('max',maxVal)
        .val(curVal), iso_holder)
    ...
    if (config.curvals.exposure_ns != undefined && config.avail.exposure_ns.length >= 2) {
      ...
      bindUpdEvents($('#range_exposure_ns')
        .attr('min',minVal)
        .attr('max',maxVal)
        .val(curVal), exposure_ns_holder)
    ...
    if (config.curvals.frame_duration != undefined && config.avail.frame_duration.length >= 2) {
      ...
      bindUpdEvents($('#range_frame_duration')
        .attr('min',minVal)
        .attr('max',maxVal)
        .val(curVal), frame_duration_holder)
    ...
    bindUpdEvents($('#range_motion_limit')
      .val(config.curvals.motion_limit), molimit_holder)
    
    bindUpdEvents($('#range_nightvision_gain')
      .val(Math.round(config.curvals.night_vision_gain * 4)), nvgain_holder)
    ...
    bindUpdEvents($('#range_nightvision_average')
      .val(config.curvals.night_vision_average), nvavg_holder)
    ...
    $("#modet_toggle").click(function() {
      if ($(this).hasClass('active')) {
        $.ajax({url: root+'/settings/motion_detect?set=off', method: "POST"})
        $(this).removeClass('active')
      } else {
        $.ajax({url: root+'/settings/motion_detect?set=on', method: "POST"})
    ...
    $("#motion_display").click(function() {
      if ($(this).hasClass('active')) {
        $.ajax({url: root+'/settings/motion_display?set=off', method: "POST"})
        $(this).removeClass('active')
      } else {
        $.ajax({url: root+'/settings/motion_display?set=on', method: "POST"})
        $(this).addClass('active')
      }
    })
  ...
  var audioCreate = function(autoPlay) {
    ...
    } else if (audioMode == 'html5_aac') {
      $('#audio_pane').append($('<audio id="html5_audio" controls><source src="'+root+'/audio.aac" type=\'audio/aac\'/></audio>'));
      if (autoPlay) {
        $('#html5_audio')[0].play();
      }
    }
  ...
  var audioDestroy = function() {
    ...
    } else if (['html5_wav', 'html5_opus', 'html5_aac'].indexOf(audioMode) != -1) {
      $('#html5_audio').remove();
    }
  }
  ...
  var videoDestroy = function () {
    if (videoMode == 'off') {
      // Do nothing
    } else if (videoMode == 'rtsp') {
      $('#rtsp_pane').css('display', 'none');
  ...
  var videoTimeout = {
    ...
    time: 4000,
    tick: function() {
      $.ajax({url: root+'/videostatus', method: "GET"})
       .done(function (info) {
...
var greetInit = function() {
  ...
  $.ajax({url: root+'/status.json?show_avail=1', method: "POST"})
   .done(function (data) {
     config = data;
     reconfigure();
   })
  ...
  $.ajax({url: root+'/modet_areas.json', method: "GET"})
    .done(function(data) {
      for (var i = 0; i<8; i++) {
        for (var j = 0; j<8; j++) {
          if (data[i][j] == 1) {
          ...
          $('#ma'+j+i).click(function() {
            ...
            if ($(this).hasClass('active')) {
              $(this).removeClass('active')
              var which = this;
              $.ajax({url: prefix + '&enabled=0', method: "POST"})
               .fail(function() {
              ...
              $(this).addClass('active')
              var which = this;
              $.ajax({url: prefix + '&enabled=1', method: "POST"})
               .fail(function() {
  ...
  $('#btn_why_lag').click(function() {
   ...
   $('.tbtn').click(function() {
    $.ajax({url: root+'/gen?event='+$(this).attr('data-num'), method: "POST"})
   })

  $('#rec_stop').click(function() {
    $.ajax({url: root+'/stopvideo?force=1', method: "POST"})
  ...
  $('#rec_button').click(function() {
    ...
    $.ajax({url: root+'/startvideo?force=1&tag='+encodeURIComponent($('#video_tag').val()), method: "POST"})
  ...
  $('#rec_circular').click(function() {
    $.ajax({url: root+'/startvideo?force=1&mode=circular&tag='+encodeURIComponent($('#video_tag').val()), method: "POST"})
      ...
      media_list.append(
        $('<div class="media file">').attr('id', fid)
              .append(
                ...
                .append($('<img style="width: 128px; height: auto;" />').attr('src', makeThumb(di, c.name) ).attr('loading', 'lazy'))
              )
              .append(
...
var videoMgrInit = function() {
  $.ajax({url: root+'/list_videos', method: "GET"})
...
var sensorsMain = function() {
  ...
  var showGps = function () {
    ...
    gpsHandle = setTimeout(function () {
      $.ajax({url: root + '/gps.json', method: "GET"})
      .done(function (data) {
        if (data.gps) {
  ...
  $.ajax({url: root + '/status.json', method: "GET"})
  .done(function(data) {
    ...
    var gpscb = $('#gpscb')
      ...
      .change(function() {
        if (gpscb.is(":checked")) {
          $.ajax({url: root+'/settings/gps_active?set=on', method: "POST"}).fail(display_error('cannot_set', 'gps')).done(function () {
            showGps();
          })
        } else {
          $.ajax({url: root+'/settings/gps_active?set=off', method: "POST"}).fail(display_error('cannot_set', 'gps'))
          hideGps();
        }
      })
  })
  ...
  $.ajax({url: root + '/sensors.json', method: "GET"}) // ?from=8999999999999999999
    .done(function(data) {
      var hasKeys = false;
